...
import com.bonade.xfh.sdk.CardsCameraActivity;
import com.bonade.xfh.sdk.ThreadManager;
import com.kernal.passportreader.sdk.base.OcrBaseActivity;
import com.bonade.xfh.sdk.utils.DefaultPicSavePath;
import com.bonade.xfh.sdk.utils.Devcode;
import com.bonade.xfh.sdk.utils.ImportRecog;
import com.bonade.xfh.sdk.utils.ManageIDCardRecogResult;
...
//                try {
//                    /**
//                     * 将Path转换成URI为了兼容Android Q
//                     */
//                    String tempCachePicturePath=getFilesDir().getPath()+"/idcard.jpg";
//                    ContentResolver cr=getContentResolver();
//                    ParcelFileDescriptor parcelFileDescriptor=cr.openFileDescriptor(uri,"r");
//                    FileDescriptor fileDescriptor= parcelFileDescriptor.getFileDescriptor();
//                    saveBitmap(BitmapFactory.decodeFileDescriptor(fileDescriptor),tempCachePicturePath);
//                    /**
//                     * 将缓存中的图片送入引擎识别
//                     */
//                    selectPath=tempCachePicturePath;
//                } catch (Exception e) {
//                    e.printStackTrace();
//                }
//
//            }else{
//                selectPath = UritoPathUtil.getImageAbsolutePath(this, uri);
//            }
            importPicToRecog(selectPath);
//            try {
//                importPicToRecog(BitmapFactory.decodeFileDescriptor(getContentResolver().openFileDescriptor(uri,"r").getFileDescriptor()));
//            } catch (FileNotFoundException e) {
//                e.printStackTrace();
//            }
     ...
     */
    @Override
    public void importPicToRecog(final Bitmap bitmap) {
        CardOcrRecogConfigure.getInstance()
                .initLanguage(getApplicationContext())
                .setnMainId(SharedPreferencesHelper.getInt(
                        getApplicationContext(), "nMainId", 2))
        ...
        ThreadManager.getThreadPool().execute(new Runnable() {
            @Override
            public void run() {
                importRecog.startImportRecogService(bitmap);
            }
        });
    }
    /**
     ...
     */
    @Override
    public void SerailNumberAuth(String authNumber) {
    ...
    /**
     * 获取证件的类型
     * @return 0：代表可以选择任何的证件类型 2:代表的是身份证的证件类型 5：代表的只能够选择中国驾驶证的证件类型
     */
    @Override
